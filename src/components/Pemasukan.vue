...
export default {
  ...
  setup() {
    ...
    // Perhatikan bahwa endpoint ini harus mengembalikan `nominal_bersih` juga,
    // atau Anda harus menghitungnya di sini jika endpoint hanya memberikan `nominal_pemasukan`.
    // Berdasarkan backend yang Anda berikan sebelumnya, `nominal_bersih` seharusnya sudah ada.
    const fetchMonthlyRevenueData = async () => {
      try {
        // Menggunakan endpoint yang sudah ada yang mengembalikan semua data pemasukan
        ...
        response.data.forEach(item => {
            ...
            if (date.getFullYear() === currentYear) {
                ...
                // PASTIKAN MENGGUNAKAN nominal_bersih DAN PARSE KE FLOAT
                monthlyAggregatedData[monthIndex] = (monthlyAggregatedData[monthIndex] || 0) + parseFloat(item.nominal_bersih) || 0;
            }
        });

        // Format data agar sesuai dengan ekspektasi chart,
        // meskipun kita akan membuat labels dan dataValues secara eksplisit di bawah.
        // Return structured data for simpler processing in renderChartMonthlyRevenue
    ...
    const initializeTabulator = async () => {
      if (pemasukanTable.value) {
        ...
        tabulatorInstance = new Tabulator(pemasukanTable.value, {
          data: data,
          layout: "fitColumns",
          ...
          columns: [
            ...
            },
            {
              ...
              formatter: function (cell) {
                return (
                  "Rp " + parseFloat(cell.getValue()).toLocaleString("id-ID")
                );
              },
              headerFilter: false,
              resizeable: false,
              ...
              formatter: (cell, formatterParams, onRendered) => {
                ...
                container
                  ...
                  .addEventListener("click", () => {
                    editIncome(id); // Call the new edit function
                  });
    ...
    // NEW: Function to render the monthly revenue chart
    const renderChartMonthlyRevenue = async () => {
      // Mengambil data agregat bulanan langsung dari fetchMonthlyRevenueData
      ...
      // Isi dataValues berdasarkan monthlyAggregatedData
      for (let i = 0; i < labels.length; i++) {
        // Ambil data untuk bulan ini (0-indexed) atau 0 jika tidak ada
      ...
      chartInstance = new Chart(ctxRevenue, {
        type: "bar",
        data: {
          labels: labels, // Fixed labels for all 12 months
          datasets: [
            {
              label: "Pendapatan Bersih (Rp)", // Label lebih jelas
              data: dataValues, // Dynamic data values from aggregated data
