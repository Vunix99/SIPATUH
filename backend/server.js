...
function createDefaultTickets(db) {
  return new Promise((resolve, reject) => {
    ...
    db.query('SELECT COUNT(*) as count FROM Tiket', (err, results) => {
      ...
      if (ticketCount > 0) {
        console.log(`Found ${ticketCount} existing tickets, skipping default creation.`);
        return resolve();
      }
...
const saveBase64Image = (base64Data, filename) => {
  try {
    ...
    console.log(`Base64 image saved: ${filename}, size: ${Math.round(buffer.length / 1024)}KB`);
    return filename;
  } catch (error) {
  ...
  return new Promise((resolve, reject) => {
    db.query(`
      ...
      CREATE TABLE IF NOT EXISTS Log_Parkir (
        ...
        waktu_keluar DATETIME ON UPDATE CURRENT_TIMESTAMP,
  ...
  try {
    ...
    app.put("/api/tiket/:id/tersedia", (req, res) => {
      ...
      if (typeof tersedia !== 'boolean') {
        return res.status(400).json({ error: 'tersedia must be boolean (true/false)' });
      }
      ...
      db.query(sql, [tersedia, id], (err, result) => {
        ...
        res.json({ message: `Tiket ${tersedia ? 'diaktifkan' : 'dinonaktifkan'}` });
      });
    });
    ...
    app.post("/api/parkirMasuk", upload.single('foto_masuk'), (req, res) => {
      ...
      if (!plat_nomor || !waktu_masuk || !nomor_tiket) {
        return res.status(400).json({ 
          error: 'Missing required fields: plat_nomor, waktu_masuk, nomor_tiket' 
        });
      }
      ...
      try {
        if (req.file) {
          ...
          console.log('Processing base64 image, size:', Math.round(foto_base64.length / 1024), 'KB');
            ...
            const insertLog = (id_kendaraan) => {
              ...
              db.beginTransaction((err) => {
                ...
                db.query(updateTiket, [id_tiket], (err) => {
                  ...
                  db.query(sql, [id_kendaraan, id_tiket, waktu_masuk, foto_filename], (err, result) => {
                    if (err) {
                      return db.rollback(() => {
    ...
    app.get("/api/parkirKeluar", (req, res) => {
      const { nomor_tiket, waktu_keluar } = req.query;
      if (!nomor_tiket || !waktu_keluar) {
        return res.status(400).json({ error: 'Missing nomor_tiket or waktu_keluar' });
      }
      ...
      db.query(getTiketId, [nomor_tiket], (err, tiketRows) => {
        ...
        db.beginTransaction((err) => {
          ...
          `;
          db.query(updateLog, [waktu_keluar, id_tiket], (err, result) => {
            if (err) {
              return db.rollback(() => {
                res.status(500).json({ error: err.message });
              });
            ...
            if (result.affectedRows === 0) {
              return db.rollback(() => {
                res.status(404).json({ error: 'Tidak ada log parkir aktif untuk tiket ini' });
              });
            }
            
            // SET TICKET BACK TO AVAILABLE
            ...
            db.query(updateTiket, [id_tiket], (err) => {
              ...
              db.commit((err) => {
                ...
                res.json({ message: "Parkir keluar tercatat, tiket tersedia kembali" });
              });
            });
          });
        });
    ...
    app.post("/api/backup", (req, res) => {
      ...
      if (!waktu_backup || !id_admin) {
        return res.status(400).json({ error: 'Missing waktu_backup or id_admin' });
      }
